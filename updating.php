...
{
    $data = getPosts();
    $insert_Query = "INSERT INTO `users`(`user_name`, `password`, `name`) VALUES ('$data[1]','$data[2]',$data[3])";
    try{
...
{
    $data = getPosts();
    $update_Query = "UPDATE `users` SET `user_name`='$data[1]',`password`='$data[2]',`name`=$data[3] WHERE `id` = $data[0]";
    try{
